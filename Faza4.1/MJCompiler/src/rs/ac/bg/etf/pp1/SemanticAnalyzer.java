...
//		}
	...
	public void visit(DesignatorStatementActPars designatorBrace) {
    	...
    	if(Obj.Meth == func.getKind()){
			report_info("Pronadjen poziv funkcije " + func.getName() + " sa "+designatorBrace.getActPars().struct.getMembersTable().numSymbols()+"/"+func.getLevel()+" argumenta na liniji " + designatorBrace.getLine(), null);
	...
	public void visit(Idents dotIdent) {
		...
		if (left.getType().getKind()==Struct.Enum || left.getType().getKind()==Struct.Class) {
			...
			if (found==Tab.noObj) {
				report_error("Greska na liniji " +  dotIdent.getParent().getLine()+ " : polje "+ dotIdent.getI2()+" nije deklarisano! na nivou:"+left.getLevel(), null);
			}
			dotIdent.obj=found;
...
//		 
	...
	public void visit (NewType newEmpty) {
		...
		if (struct.getKind()==Struct.Class) {
			...
			//currentNewClass.setLocals(struct.getMembersTable());
    	...
    	if(typeNode == Tab.noObj){
    		...
    		if(Obj.Type == typeNode.getKind()){
    			...
    			if (typeNode.getType().getKind()==Struct.Class) {
    				//type.struct.setMembers(CollectionToHash(typeNode.getLocalSymbols()));
    			}
    		}else{
...
//				}
    ...
    public static HashTableDataStructure CollectionToHash(Collection<Obj> collection) {
		...
		while (i.hasNext())
		{
		    ...
		    table.insertKey(new Obj(obj.getKind(), obj.getName(), obj.getType()));
		   	//k++;
		}
