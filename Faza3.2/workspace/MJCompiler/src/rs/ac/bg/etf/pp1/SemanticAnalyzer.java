...
//		}
	...
	public void visit(DesignatorStatementActPars designatorBrace) {
    	...
    	if(Obj.Meth == func.getKind()){
			report_info("Pronadjen poziv funkcije " + func.getName() + " sa "+currentActParams.numSymbols()+"/"+func.getLevel()+" argumenta na liniji " + designatorBrace.getLine(), null);
			...
			if (SemanticAnalyzer.equalTypeHash(myArgs, currentActParams)) {
	...
	public void visit(ActualParametres paramList) {
		int val = currentActParams.numSymbols();
		currentActParams.insertKey(new Obj(Obj.Fld, "arg_"+val, paramList.getExpr().struct));
	...
	public void visit(SingleActualParam singleParam) {
    	currentActParams=new HashTableDataStructure();
		int val = currentActParams.numSymbols();
		currentActParams.insertKey(new Obj(Obj.Fld, "arg_"+val, singleParam.getExpr().struct));
	...
	public void visit(Idents dotIdent) {
		...
		if (left.getType().getKind()==Struct.Enum) {
			
			Collection<Obj> collection = left.getLocalSymbols();
			Iterator<Obj> i  = collection.iterator();
			Obj found=Tab.noObj;
			while (i.hasNext())
			{
			...
			report_error("Greska na liniji " +  dotIdent.getParent().getLine()+ " : podrzani samo Enum tipovi. ", null);
	...
	public void visit(Exprs elem) {
		Obj left=elem.getIdentExprList().obj;
...
//		 
   ...
   public void visit(ReturnStatementExpr ret) {
    ...
    public void visit(NumberNode number) {
    	number.obj= new Obj(Obj.Con, "number", Tab.intType);
    	number.obj.setAdr(number.getNumber());
    	 
    }
	public void visit(CharNode chr) {
		chr.obj= new Obj(Obj.Con, "char", Tab.charType);
    	chr.obj.setAdr(chr.getChr());
	}
	public void visit(BooleanNode b) {
		b.obj= new Obj(Obj.Con, "bool", TabExtra.boolType);
    ...
    public static boolean equalTypeHash(SymbolDataStructure h1, SymbolDataStructure h2) {
		...
		if (h1.numSymbols() == h2.numSymbols()) {
			...
			while (itH1.hasNext() && itH2.hasNext()) {
				Obj obj1 = itH1.next();
				Obj obj2 = itH2.next();
//				if (obj1.getType().getKind()==Struct.Array && (obj1.getType().getElemType()!=obj2.getType().getElemType()))
//					return false;
